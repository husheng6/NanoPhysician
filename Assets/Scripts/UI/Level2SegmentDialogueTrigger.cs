...
/// <summary>
/// 第二关：玩家进入指定地图片段时触发剧情，每段仅触发一次。
/// </summary>
public class Level2SegmentDialogueTrigger : MonoBehaviour
{
    ...
    private static bool macrophageDialoguePlayed;
    private static bool tCellDialoguePlayed;
    ...
    {
        macrophageDialoguePlayed = false;
        tCellDialoguePlayed = false;
    }

    public static void Setup()
    {
        GameObject host = GameObject.Find("Level2SegmentDialogueTrigger");
        if (host == null)
        {
            host = new GameObject("Level2SegmentDialogueTrigger");
            host.AddComponent<Level2SegmentDialogueTrigger>();
        }
    }
    ...
    private void Start()
    {
        ...
        segments = new[]
        {
            CreateSegment("Background_2", macrophageDialoguePlayed, Level2MacrophageDialogueRunner.Play),
            CreateSegment("Background_3", tCellDialoguePlayed, Level2TCellDialogueRunner.Play)
        };
    }
        ...
        Action<Action> playDialogue)
    {
        ...
        if (segment == null)
        {
            Debug.LogWarning($"Level2SegmentDialogueTrigger: 找不到地图片段 {segmentName}。");
            return entry;
        }
    ...
    private void TriggerSegmentDialogue(SegmentEntry segment)
    {
        ...
        if (segment.SegmentName == "Background_2")
            macrophageDialoguePlayed = true;
        else if (segment.SegmentName == "Background_3")
            tCellDialoguePlayed = true;
